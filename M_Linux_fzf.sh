...
START_PATHS=( ["all"]="/di_root/10.start_all.sh" ["web"]="/di_root/11.start_apache.sh" ["was"]="/di_root/12.start_tomcat.sh" ["db"]="/di_root/13.start_db.sh" ["webAgent"]="/engn001/lenaw/1.3/bin/start_agent.sh" ["wasAgent"]="/engn001/lena/1.3/bin/start_agent.sh" ["manager"]="/engn001/lenaw/1.3/bin/start_manager.sh" )
STOP_PATHS=( ["all"]="/di_root/30.stop_all.sh" ["web"]="/di_root/31.stop_apache.sh" ["was"]="/di_root/32.stop_tomcat.sh" ["db"]="/di_root/33.stop_db.sh" ["webAgent"]="/engn001/lenaw/1.3/bin/stop_agent.sh" ["wasAgent"]="/engn001/lena/1.3/bin/stop_agent.sh" ["manager"]="/engn001/lenaw/1.3/bin/stop_manager.sh" )
LOG_PATHS=( ["web"]="/var/log/apache2/access.log" ["was"]="/opt/tomcat/latest/logs/localhost.$(date '+%Y-%m-%d').log" ["db"]="/logs001/tomcat/log.log" ["webAgent"]="/logs001/lenaw/node/weblog.log" ["wasAgent"]="/logs001/lena/node/weblog.log" ["manager"]="/logs001/lenaw/node/weblog.log" )
...
main_menu() {
    while true; do
        ...
        SELECTED=$(echo "$MENU_DATA" | fzf --height 20 --reverse --border --ansi --header="대상 서비스를 선택하세요")
